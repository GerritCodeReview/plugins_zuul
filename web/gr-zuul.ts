...
export interface CodeReviewDepends {
  depends_on_found: DependsOn[];
  depends_on_missing: DependsOn[];
  cycle: boolean;
  needed_by: NeededBy[];
}
...
export class GrZuul extends LitElement {
    ...
    return html`
      <section class="related-changes-section">
        ...
        ${this.crd.depends_on_found.map(
          (item: DependsOn) => this.renderDependsOnFound(item)
        )}
        ${this.crd.depends_on_missing.map(
          (item: DependsOn) => this.renderDependsOnMissing(item)
        )}
  ...
  private renderDependsOnFound(item: DependsOn) {
    return html`
  ...
  private createTitle(item: DependsOn | NeededBy) {
