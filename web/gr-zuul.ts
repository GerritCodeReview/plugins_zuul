...
import {ChangeInfo, RelatedChangeAndCommitInfo} from '@gerritcodereview/typescript-api/rest-api';
...
export class GrZuul extends LitElement {
    ...
    return html`
      <section class="related-changes-section">
        ...
        ${this.crd?.depends_on_found.map(
          (item: ChangeInfo) => this.renderDependsOnFound(item)
        )}
        ${this.crd?.depends_on_missing.map(
          (item: ChangeInfo) => this.renderDependsOnMissing(item)
        )}
  ...
  private renderDependsOnFound(item: ChangeInfo) {
    return html`
  ...
  private renderDependsOnMissing(item: ChangeInfo) {
    return html`
      <div class="changeContainer zuulDependencyContainer">
        <span>
          ${item}
  ...
  _computeDependencyUrl(changeInfo: ChangeInfo) {
  ...
  private createTitle(item: ChangeInfo) {
