...
import {ChangeInfo, ChangeStatus, NumericChangeId, ChangeId, RepoName} from '@gerritcodereview/typescript-api/rest-api';
...
export interface CodeReviewDepends {
  depends_on_found: ChangeInfo[];
  depends_on_missing: ChangeInfo[];
  cycle: boolean;
  needed_by: ChangeInfo[];
}

// copied from types/common.ts with pars we don't need removed.
...
export class GrZuul extends LitElement {
    ...
    return html`
      <section class="related-changes-section">
        ...
        ${this.crd?.depends_on_found.map(
          (item: ChangeInfo | RelatedChangeAndCommitInfo) => this.renderDependsOnFound(item)
        )}
        ${this.crd?.depends_on_missing.map(
          (item: ChangeInfo | RelatedChangeAndCommitInfo) => this.renderDependsOnMissing(item)
        )}
  ...
  private renderDependsOnFound(item: ChangeInfo | RelatedChangeAndCommitInfo) {
    return html`
  ...
  private renderDependsOnMissing(item: ChangeInfo | RelatedChangeAndCommitInfo) {
    return html`
      <div class="changeContainer zuulDependencyContainer">
        <span>
          ${item}
    ...
    return html`
      <section class="related-changes-section">
        ...
        ${this.crd?.needed_by.map(
          (item: ChangeInfo | RelatedChangeAndCommitInfo) => html`
            <div class="changeContainer zuulDependencyContainer">
              <a
                href=${this._computeDependencyUrl(item)}
                title=${this.createTitle(item)}
  ...
  _onChangeChanged() {
    ...
    return this.plugin.restApi().get(url).then((crd?: CodeReviewDepends) => {
      this.crd = crd;
      this.crdLoaded = true;
  ...
  _computeChangeStatusClass(change: ChangeInfo | RelatedChangeAndCommitInfo) {
  ...
  // Not all code paths are reachable, as we only have shallow ChangeInfo objects. We leave the
  // code here nonetheless, to allow for easier updating from gr-related-changes-list.js.
  _computeChangeStatus(change: ChangeInfo | RelatedChangeAndCommitInfo) {
  ...
  _computeDependencyUrl(changeInfo: ChangeInfo | RelatedChangeAndCommitInfo) {
  ...
  private createTitle(item: ChangeInfo | RelatedChangeAndCommitInfo) {
